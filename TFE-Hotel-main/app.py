...
def admin_facturas():
    filas = fetch_all(
        ...
        SELECT
            ...
            r.fecha_salida,
            r.cantidad_huespedes,
            CONCAT('RSV-', LPAD(r.id_reserva, 5, '0')) AS folio,
    ...
    for row in filas:
        noches = calcular_noches(str(row["fecha_entrada"]), str(row["fecha_salida"]))
        row["noches"]        = noches
        row["fecha_factura"] = str(row["fecha_factura"])
        row["fecha_entrada"] = str(row["fecha_entrada"])
        row["fecha_salida"]  = str(row["fecha_salida"])
    ...
    stats = {
        "total":      len(facturas),
        "ingresos":   sum(f["total"] for f in facturas if f["estado_factura"] == "pagada"),
        "pagadas":    sum(1 for f in facturas if f["estado_factura"] == "pagada"),
        "canceladas": sum(1 for f in facturas if f["estado_factura"] == "cancelada"),
    }

    return render_template("admin_facturas.html", facturas=facturas, stats=stats)


@app.route("/admin/facturas/cancelar/<int:id_factura>")
@admin_required
@role_required("gerente", "recepcionista")
def admin_cancelar_factura(id_factura):
    execute_query(
        "UPDATE FACTURAS SET estado_factura = 'cancelada' WHERE id_factura = %s",
        (id_factura,)
    )
    execute_query(
        "UPDATE PAGOS SET estado_pago = 'cancelado' WHERE id_factura = %s",
        (id_factura,)
    )
...
# PANEL DE GERENCIA
# =========================
@app.route("/admin/gerente", methods=["GET", "POST"])
@admin_required
@role_required("gerente")
def admin_gerente():
    ...
    msg_personal = msg_hab = msg_precio = msg_empresa = None
    err_personal = err_hab = err_precio = err_empresa = None

    # =========================
    # ACCIONES GET (activar/desactivar personal)
    # =========================
    if accion == "desactivar":
        uid = request.args.get("id")
        if uid:
            execute_query("UPDATE USUARIOS SET estado = 'inactivo' WHERE id_usuario = %s", (uid,))
            msg_personal = "Usuario desactivado correctamente."

    elif accion == "activar":
        uid = request.args.get("id")
        if uid:
            execute_query("UPDATE USUARIOS SET estado = 'activo' WHERE id_usuario = %s", (uid,))
            msg_personal = "Usuario activado correctamente."
    ...
    if request.method == "POST":
        ...
        if accion == "crear_personal":
            ...
            if not nombre or not usuario or not correo or not password:
                err_personal = "Todos los campos obligatorios deben completarse."
            else:
                existente = fetch_one(
                ...
                if existente:
                    err_personal = "Ya existe un usuario con ese nombre de usuario o correo."
                else:
                    execute_query(
                        """
                        INSERT INTO USUARIOS (id_rol, nombre, apellido, usuario, password, correo, estado)
                        ...
                        """,
                        (id_rol, nombre, apellido, usuario,
                         generate_password_hash(password), correo)
                    )
                    msg_personal = f"Cuenta '{usuario}' creada correctamente."

        elif accion == "crear_hab":
            numero      = request.form.get("numero", "").strip()
            observaciones = request.form.get("observaciones", "").strip()
            piso        = request.form.get("piso", "1").strip()
            id_tipo     = request.form.get("id_tipo_habitacion", "")

            if not numero or not id_tipo:
                err_hab = "El número y el tipo de habitación son obligatorios."
            else:
                existente = fetch_one(
                    "SELECT id_habitacion FROM HABITACIONES WHERE numero = %s", (numero,)
                )
                if existente:
                    err_hab = f"Ya existe una habitación con el número {numero}."
                else:
                    execute_query(
                        """
                        INSERT INTO HABITACIONES (id_tipo_habitacion, numero, piso, estado, observaciones)
                        ...
                        """,
                        (id_tipo, numero, piso, observaciones)
                    )
                    msg_hab = f"Habitación {numero} registrada correctamente."

        elif accion == "actualizar_precio":
            id_tipo      = request.form.get("id_tipo")
            nuevo_precio = request.form.get("nuevo_precio", "").strip()

            if not id_tipo or not nuevo_precio:
                err_precio = "Debes indicar el tipo y el nuevo precio."
            else:
                execute_query(
                    "UPDATE TIPOS_HABITACION SET precio_base = %s WHERE id_tipo_habitacion = %s",
                    (float(nuevo_precio), id_tipo)
                )
                msg_precio = "Precio actualizado correctamente."

        elif accion == "crear_tipo":
            nombre_tipo  = request.form.get("nombre_tipo", "").strip()
            precio_base  = request.form.get("precio_base", "").strip()
            capacidad    = request.form.get("capacidad", "2").strip()
            descripcion  = request.form.get("descripcion", "").strip()

            if not nombre_tipo or not precio_base:
                err_precio = "El nombre y el precio son obligatorios."
            else:
                execute_query(
                    """
                    INSERT INTO TIPOS_HABITACION (nombre_tipo, descripcion, capacidad, precio_base)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (nombre_tipo, descripcion, int(capacidad), float(precio_base))
                )
                msg_precio = f"Tipo '{nombre_tipo}' creado correctamente."

        elif accion == "crear_empresa":
            nombre_empresa = request.form.get("nombre_empresa", "").strip()
            correo_emp     = request.form.get("correo", "").strip()
            telefono_emp   = request.form.get("telefono", "").strip()
            direccion_emp  = request.form.get("direccion", "").strip()
            descuento      = request.form.get("descuento", "0").strip() or "0"

            if not nombre_empresa:
                err_empresa = "El nombre de la empresa es obligatorio."
            else:
                execute_query(
                    """
                    INSERT INTO EMPRESAS (nombre_empresa, direccion, telefono, correo)
                    VALUES (%s, %s, %s, %s)
                    """,
                    (nombre_empresa, direccion_emp, telefono_emp, correo_emp)
                )
                msg_empresa = f"Empresa '{nombre_empresa}' registrada correctamente."

        return redirect(url_for("admin_gerente") + f"?tab={tab}")
    ...
    personal = fetch_all(
        """
        SELECT u.id_usuario, u.nombre, u.apellido, u.usuario, u.correo, u.estado, r.nombre_rol
        FROM USUARIOS u
        INNER JOIN ROLES r ON r.id_rol = u.id_rol
    ...
    habitaciones = fetch_all(
        """
        SELECT h.id_habitacion, h.numero, h.piso, h.estado, h.observaciones,
    ...
    tipos_habitacion = fetch_all(
        "SELECT * FROM TIPOS_HABITACION ORDER BY id_tipo_habitacion"
    )

    empresas = fetch_all(
        "SELECT *, 0 AS descuento FROM EMPRESAS ORDER BY id_empresa DESC"
    )
    ...
    return render_template(
        ...
        empresas=empresas,
        msg_personal=msg_personal,
        err_personal=err_personal,
        msg_hab=msg_hab,
        err_hab=err_hab,
        msg_precio=msg_precio,
        err_precio=err_precio,
        msg_empresa=msg_empresa,
        err_empresa=err_empresa,
    )
